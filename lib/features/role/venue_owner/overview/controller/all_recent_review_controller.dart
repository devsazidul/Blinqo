...
class AllRecentReviewController extends GetxController {
  ...
  final RxList<RecentReview> reviews = <RecentReview>[].obs; // List to store reviews
  final RxBool isLoading = false.obs; // Initial loading state
  final RxBool isLoadingMore = false.obs; // Loading more reviews state
  final RxBool hasMore = true.obs; // Flag to check if more reviews are available
  ...
  Future<void> fetchAllRecentReviews({bool isLoadMore = false}) async {
    ...
    try {
      _logger.i('Fetching reviews from ${Urls.getAllRecentReviews}?page=$page&limit=$limit');
      ...
      if (response.isSuccess) {
        ...
        if (!isLoadMore) {
          EasyLoading.showSuccess('Reviews loaded successfully');
        }
      } else {
        _logger.w('Failed to fetch reviews: ${response.errorMessage}');
        EasyLoading.showError('Failed to load reviews: ${response.errorMessage}');
      }
    } catch (e, stackTrace) {
...
final List<Map<String, dynamic>> _mockData = [{
  "data": [
    {
      "id": "review_001",
      "content": "The Grand Ballroom was perfect for our gala. The staff was professional, and the ambiance was stunning. Only minor issue was parking availability.",
      "rating": 4.5,
    ...
    {
      "id": "review_002",
      "content": "Ocean View Resort was beautiful, but the event setup took longer than expected. Overall, a great experience with excellent food.",
      "rating": 4.0,
    ...
    {
      "id": "review_003",
      "content": "Tech Hub Convention Center exceeded expectations! The AV equipment was top-notch, and the venue was spacious and well-organized.",
      "rating": 5.0,
