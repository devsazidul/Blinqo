  ...
  Future<void> fetchAmenities() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<EventAmenity> amenities = data
            .map((json) => EventAmenity.fromJson(json))
            .toList();

        // Update the lists based on default selection
        availableAmenities.value = amenities.where((amenity) => !amenity.defaultSelected).toList();
        selectedAmenities.value = amenities.where((amenity) => amenity.defaultSelected).toList();
        ...
        EasyLoading.showError('Failed to load amenities: Status ${response.statusCode}');
      }
    } catch (e) {
